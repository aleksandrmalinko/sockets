...
#include <time.h> // Р±РёР±Р»РёРѕС‚РµРєР° РґР»СЏ РїРѕР»СѓРµРЅРёСЏ РІСЂРµРјРµРЅРё
...
void get_date(char *buffer, int size) {
    if (buffer == NULL)
        error("get_date(): wrong buffer pointer");
    if (size < 24)
        error("get_date(): buffer size is not enough");
...
void serve_clients(int socket) {
    ...
    int c;
    while (1) {
        c = Request(socket, buffer, BUFSIZE, &cliaddr);
        if (strncmp("date", buffer, 4) == 0) {
            get_date(buffer, BUFSIZE);
